  ...
  const getModeBadge = () => {
    switch (detectedMode) {
      ...
      case 'cross_modal_pair':
        return (
          <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/30">
            Mode 2: Optical + SAR Fusion
          </span>
        );
      case 'bi_temporal':
        return (
          <span className="px-2.5 py-1 text-xs font-semibold rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30">
            Mode 3: Bi-Temporal Change
          </span>
        );
      case 'change_vqa':
        return (
  ...
  return (
    <header className="h-16 border-b border-border bg-surface/80 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-40">
      ...
      <div className="flex items-center gap-3">
        ...
        {onOpenSatelliteSearch && (
          <button
            onClick={onOpenSatelliteSearch}
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-purple-600/90 hover:bg-purple-500 text-white text-xs font-medium font-mono transition-all shadow-md shadow-purple-600/20 active:scale-95 border border-purple-400/30"
          >
            <Search className="w-3.5 h-3.5" />
            <span>Search Copernicus</span>
          </button>
        )}

        {/* Upload Local GeoTIFF Rasters */}
        <button
          onClick={onOpenUpload}
          className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-medium font-mono transition-all shadow-md shadow-blue-600/20 active:scale-95"
        >
        ...
        {hasTrace && (
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium font-mono border border-slate-700 transition-all active:scale-95"
          >
